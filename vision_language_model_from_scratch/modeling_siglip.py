...
class SiglipAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.scale = (
            self.embed_dim**-0.5
        )  # 1/sqrt(d_k) , d_k is the dimension of the key vector
    ...
    def forward(
        ...
        attn_output = attn_output.transpose(
            1, 2
        ).contiguous()  # contiguous make the tensor in the memory to be contiguous
...
class SiglipEncoderLayer(nn.Moudle):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        hidden_states = self.mlp(
            hidden_states
        )  # add the parameters and nonlinearity to map the complex function
        # [batch_size,num_patches,embed_dim]
        hidden_states = residual + hidden_states

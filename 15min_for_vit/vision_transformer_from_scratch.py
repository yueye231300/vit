...
class MyViT(nn.Module):
    def __init__(
        ...
        self.pos_embed = nn.Parameter(
            get_positional_embeddings(n_patches**2 + 1, self.hidden_dim)
        )
        ...
        self.transformer_blocks = nn.ModuleList(
            [
                MyViTblock(hidden_dim=self.hidden_dim, n_heads=self.n_heads)
...
def main():
    ...
    model = MyViT(
        (1, 28, 28), n_patches=7, n_blocks=4, hidden_dim=8, n_heads=2, out_dim=10
    ).to(device)
    # 对于 MNIST (28x28)，使用更浅的网络结构
    # model = WaveFormer(
    #     patch_size=4,
    #     in_chans=1,
    #     num_classes=10,
